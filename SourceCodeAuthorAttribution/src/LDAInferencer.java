...
public class LDAInferencer {
	...
	public LDAInferencer(LDATrainModel trainModel){
	...
	//computes the theta of model for each document 
	public void computeTestTheta(){
		
		for(int m = 0; m < testModel.M; m++){
			for(int k = 0; k< testModel.K; k++){
				testModel.theta[m][k] = (testModel.nd[m][k] + testModel.alpha)/(testModel.ndSum[m] + testModel.K * testModel.alpha);
			}
		}
	}
	
	public void computeTestPhi(){
	
		//TODO: see later
		for(int k = 0; k < testModel.K; k++){
			for(int w = 0; w < testModel.V; w++){
				testModel.phi[k][w] = (trainModel.nw[w][k] + testModel.nw[w][k] + testModel.beta)/(trainModel.nwSum[k] + testModel.nwSum[k] + trainModel.V * testModel.beta);
			}
		}
	}
